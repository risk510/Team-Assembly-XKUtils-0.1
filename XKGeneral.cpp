...
// Value1 = "My Value"
//
BOOL XKGeneral::ReadINIFileItem(LPCSTR INIFileName, LPCSTR INISection, LPCSTR INIItem, LPSTR ItemValue, LPLONG ValueLen)
{
	BOOL retVal = FALSE;
	LONG retSize = 0;
...
//This Function simply strips out Quotes from a string.. 
//Especially if reading from INI file and value is quoted..
//Strip quotes from mostly File Paths..  hence FILENAME_MAX
void XKGeneral::StripQuotes(LPSTR sString, LPLONG strLen)
{
	...
	LPSTR tmpFirst = strchr((LPCSTR)tmpString, '\"');
	LPSTR tmpLast = strrchr((LPCSTR)tmpString, '\"');
